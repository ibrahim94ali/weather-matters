...
export class WeatherDataService {
  ...
  private readonly _weathersState = new BehaviorSubject<State>(State.UNINITIALIZED);
  ...
  getWeathers() {
    ...
    this.cityNamesList.forEach((name) => {
      ...
      this.weatherService.getWeatherByName(lat, lon).subscribe(
        ...
        if (this._weathersState.value !== State.FAILED && this._weathers.value?.length === this.cityNamesList.length) {
